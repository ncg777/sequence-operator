...
server.tool('combine', 'Combine two sequences using a combiner and an operation. Returns the result sequence as a space-separated string.', {
    ...
    content: [{ type: 'text', text: combine(c, op, x, y) }],
}));
server.tool('rotate', 'Rotate a sequence by n steps (positive = left, negative = right). Returns the rotated sequence.', {
...
server.tool('reverse', 'Reverse a sequence. Returns the reversed sequence.', {
    sequence: z.string().describe('Sequence (space-separated integers)'),
}, ({ sequence }) => ({
    content: [{ type: 'text', text: reverse(sequence) }],
}));
server.tool('cyclical_difference', 'Compute the cyclical difference of a sequence (each element minus the next, wrapping around). Returns the result sequence.', {
    sequence: z.string().describe('Sequence (space-separated integers)'),
}, ({ sequence }) => ({
    content: [{ type: 'text', text: cyclicalDifference(sequence) }],
}));
server.tool('cyclical_antidifference', 'Compute the cyclical antidifference (inverse of cyclical difference) of a sequence. Returns the result sequence.', {
    sequence: z.string().describe('Sequence (space-separated integers)'),
    k: z.number().int().default(0).describe('Starting value (default: 0)'),
}, ({ sequence, k }) => ({
    content: [{ type: 'text', text: cyclicalAntidifference(sequence, k) }],
}));
server.tool('difference', 'Compute the difference of a sequence (each element minus the previous). Returns the result sequence.', {
    sequence: z.string().describe('Sequence (space-separated integers)'),
}, ({ sequence }) => ({
    content: [{ type: 'text', text: difference(sequence) }],
}));
server.tool('antidifference', 'Compute the antidifference (cumulative sum) of a sequence. Returns the result sequence.', {
    sequence: z.string().describe('Sequence (space-separated integers)'),
    k: z.number().int().default(0).describe('Starting value (default: 0)'),
}, ({ sequence, k }) => ({
    content: [{ type: 'text', text: antidifference(sequence, k) }],
}));
server.tool('signs', 'Return the sign (+1, -1, or 0) of each element in a sequence. Returns the sign sequence.', {
    sequence: z.string().describe('Sequence (space-separated integers)'),
}, ({ sequence }) => ({
    content: [{ type: 'text', text: signs(sequence) }],
}));
server.tool('times_n', 'Sample every n-th element cyclically: result[i] = sequence[(i * n) % size]. Returns the resampled sequence.', {
    sequence: z.string().describe('Sequence (space-separated integers)'),
...
server.tool('list_operations', 'List all available operations for the combine operation.', {}, () => ({
