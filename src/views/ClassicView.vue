<template>
  <v-responsive class="align-center mx-auto pa-5" max-width="900">
            ...
            <v-card-text>
              <div class="help-content">
                ...
                <ul>
                  ...
                  <li><strong>ax²+bx+c</strong> - Rearranges the sequence so each element at position <code>x</code> is replaced by the element at index <code>(a*x² + b*x + c) mod size</code>; prompts for coefficients <code>a</code>, <code>b</code>, and <code>c</code></li>
        ...
        <!-- ax²+bx+c dialog -->
        <v-dialog v-model="showPolynomialDialog" max-width="400" class="pa-1">
          <v-card>
            <v-card-title>
              <span>ax²+bx+c</span>
              <v-btn @click="showPolynomialDialog = false" icon :style="'float:right;text-align:right;'">
                <v-icon>mdi-window-close</v-icon>
              </v-btn>
            </v-card-title>
            <v-card-text>
              <v-text-field v-model="polynomialA" label="Quadratic coefficient (a)" type="number" outlined dense></v-text-field>
              <v-text-field v-model="polynomialB" label="Linear coefficient (b)" type="number" outlined dense></v-text-field>
              <v-text-field v-model="polynomialC" label="Constant offset (c)" type="number" outlined dense></v-text-field>
            </v-card-text>
            <v-card-actions>
              <v-btn @click="applyPolynomial" color="primary" block>Apply</v-btn>
            </v-card-actions>
          </v-card>
        </v-dialog>
...
const applyPolynomial = () => {
  showPolynomialDialog.value = false;
  const a = parseInt(polynomialA.value.trim());
  if (isNaN(a)) {
    alert("Invalid quadratic coefficient. Please enter an integer.");
    return;
  }
  const b = parseInt(polynomialB.value.trim());
  if (isNaN(b)) {
    alert("Invalid linear coefficient. Please enter an integer.");
    return;
  }
  const c = parseInt(polynomialC.value.trim());
  if (isNaN(c)) {
    alert("Invalid constant offset. Please enter an integer.");
    return;
  }
  ...
  for (let x = 0; x < o.size(); x++) {
    const idx = ((a * x * x + b * x + c) % o.size() + o.size()) % o.size();
    o.set(x, s.get(idx)!);
  }
  textResult.value = o.toString();
  addResultOpToHistory('ax²+bx+c', input, textResult.value, { a, b, c });
};
